    ...
    def inlet(self, body: dict, __user__: dict = {}) -> dict:
        ...
        if self.valves.arabic_context:
            arabic_instructions = (
                ...
                "   - 'STATUS: ACTION_EXECUTED' = الإجراء تم تنفيذه فعلاً. أخبر المستخدم بالنتيجة.\n"
                "   - 'STATUS: PENDING_CONFIRMATION' = النظام يطلب تأكيد من المستخدم قبل الحذف. اعرض رسالة التأكيد.\n"
                "   - 'STATUS: CONVERSATION' = رد محادثة عادي أو تم تسجيل البيانات تلقائياً.\n\n"

                "4. ممنوع منعاً باتاً أن تقول 'تم' أو 'تمت' أو 'نجح' إلا إذا شفت 'STATUS: ACTION_EXECUTED' في رد الأداة.\n"
                "5. لو المستخدم طلب إجراء ورد الأداة 'STATUS: CONVERSATION'، "
                ...
                "7. عند رفع ملف، النظام يعالجه تلقائياً (النتائج مرفقة في الرسالة). اعرض المعلومات المستخرجة للمستخدم.\n"
                "   ⚠️ لا تستخدم الوقت الحالي كوقت الموعد — استخدم فقط التواريخ والأوقات الموجودة في المستند.\n"
                "8. ⚠️ لا تسأل المستخدم 'هل تريد أن أضيف/أسجل/أنفذ؟' — أرسل الطلب مباشرة لأداة chat ودع النظام يتعامل معه.\n"
                "   النظام يسأل عن تأكيد فقط عند الحذف. أي عملية أخرى (إضافة، تعديل، تسجيل) تُنفّذ مباشرة بدون سؤال.\n"
                "   لا تولّد 'STATUS:' من عندك — هذه الحالات تأتي فقط من رد الأداة.\n"
            )
