...
async def job_check_ha_reminders(bot: Bot):
    """Fast check (every 1 min) for HA automations only (separate from regular reminders)."""
    for tg_id, user in _tg_user_cache.items():
        try:
            ak = user.get("api_key", "")
            data = await api_get("/proactive/due-ha-automations", api_key=ak)
            ha_reminders = data.get("ha_automations", [])
            if not ha_reminders:
                continue
            ...
            for r in ha_reminders:
                ...
                # Mark notified + handle recurring HA automations
                try:
                    await api_post("/proactive/mark-notified", json={"title": r["title"]}, api_key=ak)
                except Exception:
                    pass
            ...
            logger.info("Executed %d HA automation(s) for %s", len(ha_reminders), user.get("user_id", tg_id))
        except Exception as e:
            logger.error("HA automation check for %s failed: %s", user.get("user_id", tg_id), e)
